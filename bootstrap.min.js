...
function updateCountdown() {
    ...
    if (remainingTime <= 0) {
        ...
        document.getElementById("fan").classList.remove("spin");  // Stop the fan spin
        running = false;
        return;
    }
    ...
    if (Math.floor((now - lastUpdateTime) / 1000) >= 60) {
        balance += 1;
...
function startCountdown() {
    if (running) return; // Prevent multiple countdowns

    // Start a new countdown
    targetTime = new Date().getTime() + 24 * 60 * 60 * 1000;  // Set target time (24 hours)
    lastUpdateTime = new Date().getTime();
    balance = 0;
    document.getElementById("balance").innerText = "$0.0" + balance;
    timerInterval = setInterval(updateCountdown, 1000);
    updateCountdown(); // Update the countdown immediately
    document.getElementById("fan").classList.add("spin");  // Start the fan spin
    running = true;
}
